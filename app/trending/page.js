...
import { FaSpinner } from "react-icons/fa"; // Import the spinning icon
...
const TrendingBooks = () => {
  ...
  // Fetch trending books from the Google Books API with pagination
  const fetchTrendingBooks = async () => {
    ...
    try {
      const response = await axios.get(
        `https://www.googleapis.com/books/v1/volumes?q=subject:fiction&orderBy=newest&maxResults=12&startIndex=${(page - 1) * 12}&key=${process.env.NEXT_PUBLIC_GOOGLE_BOOKS_API_KEY}`
      );
      ...
      setError("Error fetching trending books. Please try again.");
    } finally {
      setLoading(false);
    }
  };
